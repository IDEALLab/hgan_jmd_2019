...
class Model(object):
    ...
    def __init__(self, latent_dim, noise_dim, n_points, bezier_degree):
        ...
        self.discrete_latent_dim = [0, 0, 2]
...
#        x += tf.random_normal(shape=tf.shape(x), stddev=noise_std)
    ...
    def child_generator(self, c, s, z, fp, name, n_points, bezier_degree=None, reuse=tf.AUTO_REUSE):
...
#        noise_std = 0.01
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            if s is None:
                ...
                s = tf.layers.dense(s, 8)
                s = tf.layers.batch_normalization(s, momentum=0.9)
                s = tf.nn.leaky_relu(s, alpha=0.2)
                cz = tf.concat([c, s, z], axis=-1)
    ...
    def discriminator(self, x0, x1, x2, reuse=tf.AUTO_REUSE, training=True):
        ...
        with tf.variable_scope('D', reuse=reuse):
            ...
            s2 = tf.layers.dense(f012, 1024)
            s2 = tf.layers.batch_normalization(s2, momentum=0.9)
            s2 = tf.nn.leaky_relu(s2, alpha=0.2)
            s2 = tf.layers.dense(s2, 128)
            s2 = tf.layers.batch_normalization(s2, momentum=0.9)
            s2 = tf.nn.leaky_relu(s2, alpha=0.2)
            s2 = tf.layers.dense(s2, self.discrete_latent_dim[2])
            s2 = tf.nn.softmax(s2, name='s2_pred')
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        self.s2 = tf.placeholder(tf.float32, shape=[None, self.discrete_latent_dim[2]], name='s2')
        ...
        s2_target = tf.placeholder(tf.float32, shape=[None, self.discrete_latent_dim[2]])
        
        # Outputs
        d_real, _, _, _, _ = self.discriminator(self.x0, self.x1, self.x2)
        ...
        self.x1_fake = self.child_generator(self.c1, None, self.z1, f0, 'G1', self.n_points[1])
        f1 = self.encoder(self.x1_fake, 'E1', training=False)
        self.x2_fake = self.child_generator(self.c2, self.s2, self.z2, tf.concat([f0, f1], axis=-1), 'G2', self.n_points[2])
        d_fake, self.c0_fake_train, self.c1_fake_train, self.c2_fake_train, self.s2_fake_train = self.discriminator(self.x0_fake, self.x1_fake, self.x2_fake)
        
        self.d_test, self.c0_test, self.c1_test, self.c2_test, self.s2_test = self.discriminator(self.x0, self.x1, self.x2, training=False)
        ...
        qs_loss = tf.losses.softmax_cross_entropy(s2_target, self.s2_fake_train)
        ...
        d_train_fake = d_optimizer.minimize(d_loss_fake + 0.1*q_loss + qs_loss, var_list=dis_vars)
        g_train = g_optimizer.minimize(g_loss + r_loss + 0.1*q_loss + qs_loss, var_list=[g0_vars, g1_vars, g2_vars])
        ...
        tf.summary.scalar('Qs_loss', qs_loss)
        ...
        for t in range(train_steps):
            ...
            X2_discrete_latent = np.eye(2)[np.random.choice(2, batch_size)]
            X2_noise = np.random.normal(scale=0.5, size=(batch_size, self.noise_dim[2]))
            X2_fake = self.sess.run(self.x2_fake, feed_dict={self.c2: X2_latent, self.s2: X2_discrete_latent, self.z2: X2_noise, 
                                                             self.x0_fake: X0_fake, self.x1_fake: X1_fake})

            assert not (np.any(np.isnan(X0_fake)) or np.any(np.isnan(X1_fake)) or np.any(np.isnan(X2_fake)))
            
            _, dlf, qld, qsld = self.sess.run([d_train_fake, d_loss_fake, q_loss, qs_loss],
                                              feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake, self.x2_fake: X2_fake,
                                                         c0_target: X0_latent, c1_target: X1_latent, c2_target: X2_latent, s2_target: X2_discrete_latent})
            ...
            _, gl, rl, qlg, qslg = self.sess.run([g_train, g_loss, r_loss, q_loss, qs_loss],
                                                 feed_dict={self.c0: X0_latent, self.z0: X0_noise, 
                                                            self.c1: X1_latent, self.z1: X1_noise,
                                                            self.c2: X2_latent, self.s2: X2_discrete_latent, self.z2: X2_noise,
                                                            c0_target: X0_latent, c1_target: X1_latent, c2_target: X2_latent, 
                                                            s2_target: X2_discrete_latent})
            ...
            summary_str = self.sess.run(merged_summary_op, 
                                        feed_dict={self.x0: X0_real, self.x1: X1_real, self.x2: X2_real,
                                                   ...
                                                   self.s2: X2_discrete_latent, s2_target: X2_discrete_latent})
            
            summary_writer.add_summary(summary_str, t+1)
            
            # Show messages
            log_mesg = "%d: [D] real %f fake %f q %f qs %f" % (t+1, dlr, dlf, qld, qsld)
            log_mesg = "%s  [G] %f reg %f q %f qs %f" % (log_mesg, gl, rl, qlg, qslg)
            print(log_mesg)
            ...
            if save_interval>0 and (t+1)%save_interval==0:
                ...
                parents = self.synthesize_x1(1)
                synthesize_x2_0 = lambda x: self.synthesize_x2(x, 0, parents)
                synthesize_x2_1 = lambda x: self.synthesize_x2(x, 1, parents)
                plot_grid(5, gen_func=synthesize_x2_0, d=self.latent_dim[2], 
                          scale=.95, scatter=True, alpha=.7, fname='{}/x2_0'.format(save_dir))
                plot_grid(5, gen_func=synthesize_x2_1, proba_func=self.pred_proba, d=self.latent_dim[2], 
                          scale=.95, scatter=True, alpha=.7, fname='{}/x2_1'.format(save_dir))
    ...
    def restore(self, save_dir=None):
        ...
        self.s2 = graph.get_tensor_by_name('s2:0')
        ...
        self.s2_test = graph.get_tensor_by_name('D_2/s2_pred:0')
    ...
    def synthesize_x2(self, X2_latent, class_label=0, parents=None):
        if isinstance(X2_latent, int):
            N = X2_latent
            X2_latent = np.random.uniform(size=(N, self.latent_dim[2]))
            X2_discrete_latent = np.eye(2)[np.random.choice(2, N)]
            X2_noise = np.random.normal(scale=0.5, size=(N, self.noise_dim[2]))
        else:
            N = X2_latent.shape[0]
            X2_discrete_latent = np.zeros((N, 2))
            X2_discrete_latent[:,class_label] = 1
        ...
        X2 = self.sess.run(self.x2_fake, feed_dict={self.c2: X2_latent, self.s2: X2_discrete_latent, self.z2: X2_noise, self.x0_fake: X0, self.x1_fake: X1})
    ...
    def synthesize_assemblies(self, N):
        ...
        X2_discrete_latent = np.eye(2)[np.random.choice(2, N)]
        X2_noise = np.random.normal(scale=0.5, size=(N, self.noise_dim[2]))
        X2 = self.sess.run(self.x2_fake, feed_dict={self.c2: X2_latent, self.s2: X2_discrete_latent, self.z2: X2_noise, 
