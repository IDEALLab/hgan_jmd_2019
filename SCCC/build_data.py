...
def check_feasibility(superformula, circles_a, circles_b, circles_c):
    ...
    is_feasible = is_feasible_a and is_feasible_b and is_feasible_c
    return is_feasible
...
def build_data(s_points=64, c_points=16):
    ...
    for (i, superformula) in enumerate(superformulas):
        ...
        for j in range(n_c):
            ...
            is_feasible = check_feasibility(superformula, circle_a, circle_b, circle_c)
            if is_feasible:
                sc = np.concatenate((superformula, circle_a, circle_b, circle_c))
                X.append(sc)
                count_c += 1
    ...
    directory = '../results/SCCC'
    if not os.path.exists(directory):
        os.makedirs(directory)
    np.save('%s/SCCC.npy' % directory, X)
...
if __name__ == "__main__":
    ...
    visualize(X[:300, s_points+c_points:-c_points, :])
