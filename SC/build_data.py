...
def check_feasibility(superformula, circle):
...
def build_data(s_points=64, c_points=16):
    ...
    for (i, superformula) in enumerate(superformulas):
        count_c = 0
        for j in range(n_c):
            circle = random_circle(superformula, num_points=c_points)
            is_feasible = check_feasibility(superformula, circle)
            if is_feasible:
                sc = np.concatenate((superformula, circle))
                X.append(sc)
                count_c += 1
    ...
    directory = '../results/SC'
    if not os.path.exists(directory):
        os.makedirs(directory)
    np.save('%s/SC.npy' % directory, X)
