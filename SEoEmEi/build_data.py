...
def build_data(s_points=64, e_points=16):
    ...
    for (i, superformula) in enumerate(superformulas):
        ...
        for j in np.arange(len(feas_ind))[feas_ind]:
            ...
            vars_em = np.random.uniform(0.05, vars_eo[j], (n_em, 2))
            for k, var_em in enumerate(vars_em):
                ellipse_m = gen_ellipse(var_em[0], var_em[1], num_points=e_points)
                vars_ei = np.random.uniform(0.05, vars_em[k], (n_ei, 2))
                for var_ei in vars_ei:
                    ellipse_i = gen_ellipse(var_ei[0], var_ei[1], num_points=e_points)
                    se = np.concatenate((superformula, ellipse_o, ellipse_m, ellipse_i))
                    X.append(se)
            count_eo += 1
            if count_eo == n_eo:
                break
    ...
    directory = '../results/SEoEmEi'
    if not os.path.exists(directory):
        os.makedirs(directory)
    np.save('%s/SEoEmEi.npy' % directory, X)
...
if __name__ == "__main__":
    ...
    visualize(X[:300, s_points+e_points:-e_points, :])
