...
#        noise_std = 0.01
    ...
    def discriminator(self, x0, x1, x2, x3, reuse=tf.AUTO_REUSE, training=True):
        ...
        with tf.variable_scope('D', reuse=reuse):
            ...
            f0123 = tf.concat([f0, f1, f2, f3], axis=-1)
            d = tf.layers.dense(f0123, 1024)
    ...
    def embed(self, X0, X1, X2, X3):
        ...
        X2 = preprocess(X2)
        X3 = preprocess(X3)
        X0_latent, X1_latent, X2_latent, X3_latent = self.sess.run([self.c0_test, self.c1_test, self.c2_test, self.c3_test], 
                                                                   feed_dict={self.x0: X0, self.x1: X1, self.x2: X2, self.x3: X3})
        return X0_latent[:,0,:], X1_latent[:,0,:], X2_latent[:,0,:], X3_latent[:,0,:]
