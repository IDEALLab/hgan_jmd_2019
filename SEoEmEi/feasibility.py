import numpy as np
from SEoEmEi.build_data import filt_se
...
def check_feasibility(X0, X1, X2, X3):
    ...
    is_feasibe3 = np.linalg.norm(center1) < 1e-2
    is_feasibe4 = np.linalg.norm(center2) < 1e-2
    is_feasibe5 = np.linalg.norm(center3) < 1e-2
    is_feasibe = is_feasibe0 and is_feasibe1 and is_feasibe2 and is_feasibe3 and is_feasibe4 and is_feasibe5
    return is_feasibe


if __name__ == '__main__':
    
    X = np.load('../results/SEoEmEi/SEoEmEi.npy')
    X0 = X[:,:64]
    X1 = X[:,64:128]
    X2 = X[:,128:-64]
    X3 = X[:,-64:]
