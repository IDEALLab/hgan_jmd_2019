...
#        noise_std = 0.01
    ...
    def discriminator(self, x0, x1, x2, reuse=tf.AUTO_REUSE, training=True):
        ...
        with tf.variable_scope('D', reuse=reuse):
            ...
            f012 = tf.concat([f0, f1, f2], axis=-1)
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        # Outputs
        d_real = self.discriminator(self.x0, self.x1, self.x2)
        ...
        self.x2_fake = self.child_generator(self.c2, self.z2, f0, 'G2', self.n_points[2])
        d_fake = self.discriminator(self.x0_fake, self.x1_fake, self.x2_fake)
        
        self.d_test = self.discriminator(self.x0, self.x1, self.x2, training=False)
        ...
        g_train = g_optimizer.minimize(g_loss + r_loss, var_list=[g0_vars, g1_vars, g2_vars])
        ...
        for t in range(train_steps):
            ...
            X2_fake = self.sess.run(self.x2_fake, feed_dict={self.c2: X2_latent, self.z2: X2_noise, self.x0_fake: X0_fake})
    ...
    def synthesize_x2(self, X2_latent, parents=None):
        ...
        X2 = self.sess.run(self.x2_fake, feed_dict={self.c2: X2_latent, self.z2: X2_noise, self.x0_fake: X0})
