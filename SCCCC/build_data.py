...
def check_feasibility(superformula, circles_a, circles_b, circles_c, circles_d):
    is_feasible_a = check_feasibility_sc(superformula, circles_a)
    is_feasible_b = check_feasibility_sc(superformula, circles_b)
    is_feasible_c = check_feasibility_sc(superformula, circles_c)
    is_feasible_d = check_feasibility_sc(superformula, circles_d)
    is_feasible = is_feasible_a and is_feasible_b and is_feasible_c and is_feasible_d
    return is_feasible
...
def build_data(s_points=64, c_points=16):
    ...
    for (i, superformula) in enumerate(superformulas):
        ...
        for j in range(n_c):
            ...
            circle_c = random_circle(superformula, num_points=c_points)
            circle_d = random_circle(superformula, num_points=c_points, outside=True)
            is_feasible = check_feasibility(superformula, circle_a, circle_b, circle_c, circle_d)
            if is_feasible:
                sc = np.concatenate((superformula, circle_a, circle_b, circle_c, circle_d))
                X.append(sc)
                count_c += 1
    ...
    directory = '../results/SCCCC'
    if not os.path.exists(directory):
        os.makedirs(directory)
    np.save('%s/SCCCC.npy' % directory, X)
...
if __name__ == "__main__":
    ...
    X = build_data(s_points, c_points)
    visualize(X[:300, :s_points, :])
    visualize(X[:300, s_points:s_points+c_points, :])
    visualize(X[:300, s_points+c_points:s_points+2*c_points, :])
    visualize(X[:300, s_points+2*c_points:-c_points, :])
    visualize(X[:300, -c_points:, :])
    visualize(X[:300, :, :])
