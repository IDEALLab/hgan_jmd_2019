...
#        noise_std = 0.01
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        self.X1_train = X_train[:, self.n_points[0]:]
        ...
        # Outputs
        d_real, _ = self.discriminator(self.x0, self.x1)
        
        self.x0_fake, cp_train, w_train, self.x1_fake = self.generator(self.c, self.z, 'G')
        d_fake, self.c_fake_train = self.discriminator(self.x0_fake, self.x1_fake)
        
        self.d_test, self.c_test = self.discriminator(self.x0, self.x1, training=False)
        ...
        for t in range(train_steps):
            ...
            _, dlr = self.sess.run([d_train_real, d_loss_real], feed_dict={self.x0: X0_real, self.x1: X1_real})
            ...
            X0_fake, X1_fake = self.sess.run([self.x0_fake, self.x1_fake], feed_dict={self.c: X_latent, self.z: X_noise})

            assert not (np.any(np.isnan(X0_fake)) or np.any(np.isnan(X1_fake)))
            
            _, dlf, qld = self.sess.run([d_train_fake, d_loss_fake, q_loss],
                                        feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake, c_target: X_latent})
            ...
            summary_str = self.sess.run(merged_summary_op, 
                                        feed_dict={self.x0: X0_real, self.x1: X1_real,
    ...
    def synthesize(self, X_latent):
        ...
        X0, X1 = self.sess.run([self.x0_fake, self.x1_fake], feed_dict={self.c: X_latent, self.z: X_noise})
        return [postprocess(X0), postprocess(X1)]
    
    def synthesize_assemblies(self, X_latent):
        X0, X1 = self.synthesize(X_latent)
        assemblies = np.concatenate((X0, X1), axis=1)
        return postprocess(assemblies)
    
    def embed(self, X0, X1):
        X0 = preprocess(X0)
        X1 = preprocess(X1)
        X_latent = self.sess.run(self.c_test, feed_dict={self.x0: X0, self.x1: X1})
        return X_latent[:,0,:]
    
    def pred_proba(self, X0, X1):
        X0 = preprocess(X0)
        X1 = preprocess(X1)
        logits = self.sess.run(self.d_test, feed_dict={self.x0: X0, self.x1: X1}).flatten()
