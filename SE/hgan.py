...
#        noise_std = 0.01
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        # Outputs
        d_real, _, _ = self.discriminator(self.x0, self.x1)
        ...
        d_fake, self.c0_fake_train, self.c1_fake_train = self.discriminator(self.x0_fake, self.x1_fake)
        
        self.d_test, self.c0_test, self.c1_test = self.discriminator(self.x0, self.x1, training=False)
        ...
        g_train = g_optimizer.minimize(g_loss + r_loss + 0.1*q_loss, var_list=[g0_vars, g1_vars])
        ...
        for t in range(train_steps):
            ...
            _, dlf, qld = self.sess.run([d_train_fake, d_loss_fake, q_loss],
                                        feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake,
                                                   c0_target: X0_latent, c1_target: X1_latent})
    ...
    def embed(self, X0, X1):
        X0 = preprocess(X0)
        X1 = preprocess(X1)
        X0_latent, X1_latent = self.sess.run([self.c0_test, self.c1_test], 
                                             feed_dict={self.x0: X0, self.x1: X1})
        return X0_latent[:,0,:], X1_latent[:,0,:]
