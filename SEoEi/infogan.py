...
#        noise_std = 0.01
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        # Outputs
        d_real, _ = self.discriminator(self.x0, self.x1, self.x2)
        
        self.x0_fake, cp_train, w_train, self.x1_fake, self.x2_fake = self.generator(self.c, self.z, 'G')
        d_fake, self.c_fake_train = self.discriminator(self.x0_fake, self.x1_fake, self.x2_fake)
        
        self.d_test, self.c_test = self.discriminator(self.x0, self.x1, self.x2, training=False)
        ...
        for t in range(train_steps):
            ...
            X0_fake, X1_fake, X2_fake = self.sess.run([self.x0_fake, self.x1_fake, self.x2_fake], feed_dict={self.c: X_latent, self.z: X_noise})
            ...
            _, dlf, qld = self.sess.run([d_train_fake, d_loss_fake, q_loss],
                                        feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake, self.x2_fake: X2_fake, c_target: X_latent})
    ...
    def synthesize(self, X_latent):
        ...
        X0, X1, X2 = self.sess.run([self.x0_fake, self.x1_fake, self.x2_fake], feed_dict={self.c: X_latent, self.z: X_noise})
        return [postprocess(X0), postprocess(X1), postprocess(X2)]
    
    def synthesize_assemblies(self, X_latent):
        X0, X1, X2 = self.synthesize(X_latent)
    ...
    def embed(self, X0, X1, X2):
        ...
        X1 = preprocess(X1)
        X2 = preprocess(X2)
        X_latent = self.sess.run(self.c_test, feed_dict={self.x0: X0, self.x1: X1, self.x2: X2})
        return X_latent[:,0,:]
...
if __name__ == '__main__':
    ...
    model.restore(save_dir='../results/SEoEi/infogan/10000/')
