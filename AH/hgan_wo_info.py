"""
Hierarchical GAN without mutual information losses
...
#        noise_std = 0.01
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        # Outputs
        d_real = self.discriminator(self.x0, self.x1)
        ...
        d_fake = self.discriminator(self.x0_fake, self.x1_fake)
        
        self.d_test = self.discriminator(self.x0, self.x1, training=False)
        ...
        d_train_fake = d_optimizer.minimize(d_loss_fake, var_list=dis_vars)
        g_train = g_optimizer.minimize(g_loss + r_loss, var_list=[g0_vars, g1_vars])
        ...
        for t in range(train_steps):
            ...
            _, dlf = self.sess.run([d_train_fake, d_loss_fake],
                                   feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake})
            ...
            _, gl, rl = self.sess.run([g_train, g_loss, r_loss],
            ...
            summary_str = self.sess.run(merged_summary_op, 
                                        feed_dict={self.x0: X0_real, self.x1: X1_real,
                                                   ...
                                                   self.c0: X0_latent, self.z0: X0_noise, 
                                                   self.c1: X1_latent, self.z1: X1_noise})
            ...
            log_mesg = "%s  [G] %f reg %f" % (log_mesg, gl, rl)
            print(log_mesg)
