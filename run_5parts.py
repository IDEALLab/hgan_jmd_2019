...
if __name__ == "__main__":
    ...
    if  args.data == 'SEoEmEmEi':
        data_fname = 'results/SEoEmEmEi/SEoEmEmEi.npy'
        latent_dim = [2, 2, 2, 2, 2]
        noise_dim = [10, 0, 0, 0, 0]
    ...
    X1_test = X_test[:,n_points[0]:sum(n_points[:2])]
    X2_test = X_test[:,sum(n_points[:2]):sum(n_points[:3])]
    X3_test = X_test[:,sum(n_points[:3]):-n_points[4]]
    ...
    X3, X0, X1, X2 = model.synthesize_x3(1)
    ...
    print('Plotting synthesized x4 ...')
    synthesize_x4 = lambda x: model.synthesize_x4(x, [X0, X1, X2, X3])
    plot_grid(5, gen_func=synthesize_x4, d=latent_dim[4], scale=.95, 
              scatter=False, alpha=.7, c='k', fname='{}/x4'.format(results_dir))
    ...
    results_mesg_7 = 'Consistency for latent space 4: %.3f +/- %.3f' % (cons4_mean, cons4_err)
