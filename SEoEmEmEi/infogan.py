...
#        noise_std = 0.01
    ...
    def ellipse_net(self, inputs, n_points, name, reuse=tf.AUTO_REUSE):
    ...
    def generator(self, c, z, name, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            dp1 = self.ellipse_net(cz, self.n_points[1], 'x1')
            dp2 = self.ellipse_net(cz, self.n_points[2], 'x2')
            dp3 = self.ellipse_net(cz, self.n_points[3], 'x3')
            dp4 = self.ellipse_net(cz, self.n_points[4], 'x4')
            
            return dp0, cp, w, dp1, dp2, dp3, dp4
    ...
    def discriminator(self, x0, x1, x2, x3, x4, reuse=tf.AUTO_REUSE, training=True):
        ...
        with tf.variable_scope('D', reuse=reuse):
            ...
            f = tf.concat([f0, f1, f2, f3, f4], axis=-1)
    ...
    def train(self, X_train, train_steps=2000, batch_size=256, save_interval=0, save_dir=None):
        ...
        # Outputs
        d_real, _ = self.discriminator(self.x0, self.x1, self.x2, self.x3, self.x4)
        
        self.x0_fake, cp_train, w_train, self.x1_fake, self.x2_fake, self.x3_fake, self.x4_fake = self.generator(self.c, self.z, 'G')
        d_fake, self.c_fake_train = self.discriminator(self.x0_fake, self.x1_fake, self.x2_fake, self.x3_fake, self.x4_fake)
        
        self.d_test, self.c_test = self.discriminator(self.x0, self.x1, self.x2, self.x3, self.x4, training=False)
        ...
        for t in range(train_steps):
            ...
            X0_fake, X1_fake, X2_fake, X3_fake, X4_fake = self.sess.run([self.x0_fake, self.x1_fake, self.x2_fake, self.x3_fake, self.x4_fake], feed_dict={self.c: X_latent, self.z: X_noise})
            ...
            _, dlf, qld = self.sess.run([d_train_fake, d_loss_fake, q_loss],
                                        feed_dict={self.x0_fake: X0_fake, self.x1_fake: X1_fake, self.x2_fake: X2_fake, self.x3_fake: X3_fake, self.x4_fake: X4_fake, c_target: X_latent})
            ...
            summary_str = self.sess.run(merged_summary_op, 
                                        feed_dict={self.x0: X0_real, self.x1: X1_real, self.x2: X2_real, self.x3: X3_real, self.x4: X4_real,
                                                   self.x0_fake: X0_fake, self.x1_fake: X1_fake, self.x2_fake: X2_fake, self.x3_fake: X3_fake, self.x4_fake: X4_fake, 
    ...
    def restore(self, save_dir=None):
        ...
        self.x4_fake = graph.get_tensor_by_name('G/x4/x_fake:0')
    ...
    def synthesize(self, X_latent):
        ...
        X0, X1, X2, X3, X4 = self.sess.run([self.x0_fake, self.x1_fake, self.x2_fake, self.x3_fake, self.x4_fake], feed_dict={self.c: X_latent, self.z: X_noise})
        return [postprocess(X0), postprocess(X1), postprocess(X2), postprocess(X3), postprocess(X4)]
    
    def synthesize_assemblies(self, X_latent):
        X0, X1, X2, X3, X4 = self.synthesize(X_latent)
    ...
    def embed(self, X0, X1, X2, X3, X4):
        ...
        X3 = preprocess(X3)
        X4 = preprocess(X4)
        X_latent = self.sess.run(self.c_test, feed_dict={self.x0: X0, self.x1: X1, self.x2: X2, self.x3: X3, self.x4: X4})
        return X_latent[:,0,:]
